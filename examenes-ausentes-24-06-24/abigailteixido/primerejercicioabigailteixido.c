...
int verif(N){
    if(N<=0){
        printf("error, introduzca el valor otra vez");/* verifica la variable por primera vez */
        scanf("%d", &N);
    }
    if(N<=0){
...
void verificarpuntajes(cantidad){
    int j;
    int igual;
    int array[cantidad*5][3]; /*se asume que cada alumno realizo las mismas tareas, entonces, se procede a inciializar una matriz que tendra por fila: cedula, puntaje asignado y obtenido*/
    int obtenido;
    int asignado;
    int i;
    int cedula;
    int cedulas[5];
    for(i=0; i<5; i++){
        printf("introduzca el número de cedula del estudiante:");
        scanf("%d", &cedulas[i]);
        for(j=0; j<i; j++){
    ...
    for(i=0; i<cantidad*5; i++){
        printf("introduzca el CI del estudiante"); /*lectura e introduccion respectiva de los datos*/
        scanf("%d", &cedula);
        cedula = verif(cedula);
        for(j=0; j<i; j++){
            if(cedulas[i]==cedulas[j]){
        ...
        while(asignado<obtenido || obtenido<=0){
            printf("error, intente de nuevo"); /*validacion para que el puntaje obtenido no sea mayor que el asignado*/
            scanf("%d", &obtenido);
        }
        array[i][2]=obtenido;
    ...
    } /*ordenar la matriz para que todas las tareas de un mismo alumno estén juntas*/
