...
// m_fgets no tiene en cuenta la el caracter nulo al final, leerá como mucho n -
// 1 carcteres *y* el caracter nulo, retorna la cantidad de bytes que se
// modificaron en arr, guarda si se ha leído una coma en hay_coma
// Si coma es 2, entonces hay más de una coma

// Esta estructura es bastante innecesaria, pero porque sí nms
struct Nombre {
  char *nom, *ap;
  struct Nombre *next;
};
...
// 'lim' es límite para ambos 'arr_nom' y 'arr_ap'
int m_fgets(char *arr_nom, char *arr_ap, int lim) {
  int c, i = 0;
  while ((c = getchar()) == ' ' || c == '\t' || c == '\n')
    ;
  ...
  do {
    ...
    if (i == lim) {
      printf("Se ha alcanzado la longitud máxima para arr_nom\n");
      return 1;
    }
  } while ((c = getchar()) != '\n' && c != ' ' && c != ',');

  if ('\n' == c) {
    fprintf(stderr, "Se ha ingresado un salto de línea antes de ingresar la "
                    "coma para denotar el apellido\n");
    return 1;
  }
  ...
  if (c != ',')
    while ((c = getchar()) == ' ' || c == '\t' || c == '\n') {
      if ('\n' == c) {
        fprintf(stderr,
                "Se ha ingresado un salto de línea antes de ingresar la "
                "coma para denotar el apellido\n");
        return 1;
      }
    }
  ...
  if (c != ',') {
    fprintf(stderr, "Se ha ingresado un segundo nombre\n");
    return 1;
  }
  ...
  if ('\n' == c) {
    fprintf(stderr,
            "Se ha ingresado un salto de línea en lugar del apellido\n");
    return 1;
  }

  if (',' == c) {
    fprintf(stderr, "Se ha ingresado una coma en lugar del apellido\n");
    return 1;
  }
...
do {
  ...
  ++i;
  if (i == lim) {
    printf("Se ha alcanzado la longitud máxima para arr_ap\n");
    return 1;
  }
} while ((c = getchar()) != '\n' && c != ' ' && c != ',' && c != '\t');
...
if ('\n' != c) {
  fprintf(stderr, "Se ingresó otra cosa más después del apellido\n");
  return 1;
}
